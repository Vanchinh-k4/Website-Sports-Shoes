...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["save_slider"])) {
  $title = $_POST["tieude_slider"];
  $subtitle = $_POST["tieudecon_slider"];
  $title_img_name = basename($_FILES["tieude_slider_img"]["name"]);
  ...
  $imageFileType = strtolower(pathinfo($title_img_name, PATHINFO_EXTENSION));
    ...
    if (isset($_POST["save_slider"])) {
        $check = getimagesize($_FILES["tieude_slider_img"]["tmp_name"]);
        if ($check !== false) {
            echo " - " . $check["mime"] . ".";
            $uploadOk = 1;
        } else {
    ...
    if ($_FILES["tieude_slider_img"]["size"] > 500000) {
            echo "<script>
            Swal.fire({
                icon: 'error',
  ...
  if ($uploadOk != 0) {
      if (move_uploaded_file($_FILES["tieude_slider_img"]["tmp_name"], $target_file)) {
          $title_img_path = $target_file;

          $sql = "INSERT INTO tbl_slider (caption_title, caption_subtitle, img) VALUES (?, ?, ?) ";
          $stmt = $con->prepare($sql);
          $stmt->bind_param("sss", $title, $subtitle, $title_img_path);
          if($stmt->execute())
              echo "<script>
                    Swal.fire({
                        icon: 'success',
                      ...
                      window.location.href = 'form-add-slider.php';
                    });
                </script>";
          } else {
              echo "<script>
      ...
      <div class="col-md-12">
        <div class="tile">
          ...
          <div class="tile-body">
            ...
            <form class="row" action="form-add-slider.php" method="post" enctype="multipart/form-data">
              <div class="form-group col-md-3">
                <label class="control-label">Tiêu đề Slider </label>
                <input class="form-control" type="text" placeholder="" name="tieude_slider" required>
              </div>
              <div class="form-group col-md-6">
                <label class="control-label">Tên tiêu đề con </label>
                <input class="form-control" type="text" placeholder="" name="tieudecon_slider" required>
              </div>
              ...
              <div class="form-group col-md-3">
                ...
                <div id="myfileupload">
                  <input type="file" id="uploadfile" name="tieude_slider_img" required onchange="readURL(this);" />
                </div>
                <div id="thumbbox">
              ...
              <button class="btn btn-save" type="submit" name="save_slider">Lưu lại</button>
              <a class="btn btn-cancel" href="quanlimenu/menungang.php">Hủy bỏ</a>
              </form>
          </div>
        </div>
