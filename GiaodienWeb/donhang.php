     ...
     <div class="container">
        <div class="profile-layout">
            <div class="profile-siderbar">
                ...
                <div class="profile-siderbar-menu-item">
                  <a href="#" id="donhang"><span class="fas fa-file-alt"></span> Đơn mua</a>
                </div>
                <div class="profile-siderbar-menu-item">
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $ngaymua_dinhdang = date('H:i:s - d/m/Y', $timestamp);
        foreach ($hang_duoc_mua_array as $item) {
          ...
          $madon_with_hashtag='#22CSHOP'.$madon_uppercare;
            ...
            if (count($detail) >= 3) {
                ...
                $query_sanpham = "SELECT sanpham_name, sanpham_giasale
                                  FROM tbl_sanpham 
                                  WHERE sanpham_id = ?";
                ...
                if ($result_sanpham && $result_sanpham->num_rows > 0) {
                    ...
                    while ($row_sanpham = $result_sanpham->fetch_assoc()) {
                        ...
                        $gia = number_format(floatval($row_sanpham['sanpham_giasale']), 0, ',', '.').'đ';

                        echo '<tr>'; 
                        echo '<td>'.$madon_with_hashtag.'</td>';
                        echo '<td width="350px" ><span>'.$ngaymua_dinhdang.'</span></td>';
                        echo '<td class="status-column">';
                        if($noidung==$daduyet){
                            echo '<span class="badge" id="bg_infosuccess">'.$daduyet.'</span>';
                        }else if($noidung!=$daduyet){
                            echo '<span class="badge" id="bg_info" >'.$noidung.'</span>';
                        }
                        echo '</td>';
                        echo '<td><a href="chitietdonhang.php?product_detail_id='.$productId.'"><i class="fa-solid fa-eye"></a></iclass=></td>';
                        echo '<td>';
                        if ($noidung != $daduyet) {
                            echo '<button type="button" class="remove-order" data-product-id="'.$productId.'" data-product-quantity="'.$soluong.'" data-product-size="'.$kichco.'" data-id="'.$id_donhang.'">Hủy</button>';
                        }
                        echo '</td>';
                        echo '</tr>';
  ...
  echo '<td colspan="6" style="font-size: 20px; color:red"> Chưa có đơn hàng nào! </td>';
}
?>
                </tbody>
            </table>
